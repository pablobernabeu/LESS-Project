...
# Analysis of EEG data for gender agreement within time window of 200-500 ms and 
...
EEG_genderAgr_200_500_lateral_data <- EEG_genderAgr_200_500_lateral_data %>%
...
# Save data to disk
saveRDS(EEG_genderAgr_200_500_lateral_data, 
        'data/merged data/EEG_genderAgr_200_500_lateral_data.rds')

# MODEL
# Measure running time
system.time({
  
  EEG_genderAgr_200_500_lateral_lmerTest <- lmerTest::lmer(
...
saveRDS(EEG_genderAgr_200_500_lateral_lmerTest, 
        'analyses/EEG/results/EEG_genderAgr_200_500_lateral_lmerTest.rds')

# Calculate p values using Satterthwaite method (Luke, 2017; 
# https://doi.org/10.3758/s13428-016-0809-y)
summary(EEG_genderAgr_200_500_lateral_lmerTest, ddf = 'Satterthwaite') %>%
  saveRDS('analyses/EEG/results/Satterthwaite_summary_EEG_genderAgr_200_500_lateral_lmerTest.rds')
...
# MuMIn::r.squaredGLMM(EEG_genderAgr_200_500_lateral_lmerTest) %>%
#   saveRDS('analyses/EEG/results/Nakagawa2017_R2_EEG_genderAgr_200_500_lateral_lmerTest.rds')

# Calculate 95% confidence intervals of fixed effects
lme4::confint.merMod(
  EEG_genderAgr_200_500_lateral_lmerTest, method = 'Wald',
  # Compute 95% CIs for every effect, as well as for the intercept
  parm = rownames(summary(EEG_genderAgr_200_500_lateral_lmerTest)$coefficients)
) %>%
  saveRDS('analyses/EEG/results/confint_EEG_genderAgr_200_500_lateral_lmerTest.rds')

# Save random effects
lme4::ranef(EEG_genderAgr_200_500_lateral_lmerTest) %>%
  saveRDS('analyses/EEG/results/ranef_EEG_genderAgr_200_500_lateral_lmerTest.rds')
