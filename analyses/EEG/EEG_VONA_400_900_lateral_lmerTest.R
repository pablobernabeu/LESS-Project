...
EEG_VONA_400_900_lateral_data <- EEG_VONA_400_900_lateral_data %>%
...
# Save data to disk
saveRDS(EEG_VONA_400_900_lateral_data, 
        'data/merged data/EEG_VONA_400_900_lateral_data.rds')

# MODEL
# Measure running time
system.time({
  
  EEG_VONA_400_900_lateral_lmerTest <- lmerTest::lmer(
    ...
    data = EEG_VONA_400_900_lateral_data,
...
saveRDS(EEG_VONA_400_900_lateral_lmerTest, 
        'analyses/EEG/results/EEG_VONA_400_900_lateral_lmerTest.rds')

# Calculate p values using Satterthwaite method (Luke, 2017; 
# https://doi.org/10.3758/s13428-016-0809-y)
summary(EEG_VONA_400_900_lateral_lmerTest, ddf = 'Satterthwaite') %>%
  saveRDS('analyses/EEG/results/Satterthwaite_summary_EEG_VONA_400_900_lateral_lmerTest.rds')
...
# MuMIn::r.squaredGLMM(EEG_VONA_400_900_lateral_lmerTest) %>%
#   saveRDS('analyses/EEG/results/Nakagawa2017_R2_EEG_VONA_400_900_lateral_lmerTest.rds')

# Calculate 95% confidence intervals of fixed effects
lme4::confint.merMod(
  EEG_VONA_400_900_lateral_lmerTest, method = 'Wald',
  # Compute 95% CIs for every effect, as well as for the intercept
  parm = rownames(summary(EEG_VONA_400_900_lateral_lmerTest)$coefficients)
) %>%
  saveRDS('analyses/EEG/results/confint_EEG_VONA_400_900_lateral_lmerTest.rds')

# Save random effects
lme4::ranef(EEG_VONA_400_900_lateral_lmerTest) %>%
  saveRDS('analyses/EEG/results/ranef_EEG_VONA_400_900_lateral_lmerTest.rds')
