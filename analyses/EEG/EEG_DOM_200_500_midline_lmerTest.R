...
EEG_DOM_200_500_midline_data <- EEG_DOM_200_500_midline_data %>%
...
# Save data to disk
saveRDS(EEG_DOM_200_500_midline_data, 
        'data/merged data/EEG_DOM_200_500_midline_data.rds')

# MODEL
# Measure running time
system.time({
  
  EEG_DOM_200_500_midline_lmerTest <- lmerTest::lmer(
    ...
    data = EEG_DOM_200_500_midline_data,
...
saveRDS(EEG_DOM_200_500_midline_lmerTest, 
        'analyses/EEG/results/EEG_DOM_200_500_midline_lmerTest.rds')

# Calculate p values using Satterthwaite method (Luke, 2017; 
# https://doi.org/10.3758/s13428-016-0809-y)
summary(EEG_DOM_200_500_midline_lmerTest, ddf = 'Satterthwaite') %>%
  saveRDS('analyses/EEG/results/Satterthwaite_summary_EEG_DOM_200_500_midline_lmerTest.rds')
...
# MuMIn::r.squaredGLMM(EEG_DOM_200_500_midline_lmerTest) %>%
#   saveRDS('analyses/EEG/results/Nakagawa2017_R2_EEG_DOM_200_500_midline_lmerTest.rds')

# Calculate 95% confidence intervals of fixed effects
lme4::confint.merMod(
  EEG_DOM_200_500_midline_lmerTest, method = 'Wald',
  # Compute 95% CIs for every effect, as well as for the intercept
  parm = rownames(summary(EEG_DOM_200_500_midline_lmerTest)$coefficients)
) %>%
  saveRDS('analyses/EEG/results/confint_EEG_DOM_200_500_midline_lmerTest.rds')

# Save random effects
lme4::ranef(EEG_DOM_200_500_midline_lmerTest) %>%
  saveRDS('analyses/EEG/results/ranef_EEG_DOM_200_500_midline_lmerTest.rds')
