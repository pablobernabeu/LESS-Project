...
EEG_genderAgr_MiniNor_400_900_lateral_data <- EEG_genderAgr_MiniNor_400_900_lateral_data %>%
  left_join(baseline_predictor, by = c('participant_lab_ID', 'brain_region', 'sentence_marker'))

# # Save data to disk (caution: LARGE file!)
# saveRDS(EEG_genderAgr_MiniNor_400_900_lateral_data, 
#         'data/final data/EEG_genderAgr_MiniNor_400_900_lateral_data.rds')

# MODEL
# Measure running time
system.time({
  
  EEG_genderAgr_MiniNor_400_900_lateral_lmerTest <- lmerTest::lmer(
    ...
    data = EEG_genderAgr_MiniNor_400_900_lateral_data,
...
saveRDS(EEG_genderAgr_MiniNor_400_900_lateral_lmerTest, 
        'analyses/results/EEG_genderAgr_MiniNor_400_900_lateral_lmerTest.rds')

# Calculate p values using Kenward-Roger method (Luke, 2017; 
# https://doi.org/10.3758/s13428-016-0809-y)
summary(EEG_genderAgr_MiniNor_400_900_lateral_lmerTest, ddf <- 'Kenward-Roger') %>%
  saveRDS('analyses/results/KR_summary_EEG_genderAgr_MiniNor_400_900_lateral_lmerTest.rds')
...
# MuMIn::r.squaredGLMM(EEG_genderAgr_MiniNor_400_900_lateral_lmerTest) %>%
#   saveRDS('analyses/results/Nakagawa2017_R2_EEG_genderAgr_MiniNor_400_900_lateral_lmerTest.rds')

# Calculate 95% confidence intervals of fixed effects
lme4::confint.merMod(
  EEG_genderAgr_MiniNor_400_900_lateral_lmerTest, method <- 'profile',
  # Compute 95% CIs for every effect, as well as for the intercept
  parm <- rownames(summary(EEG_genderAgr_MiniNor_400_900_lateral_lmerTest)$coefficients)
) %>%
  saveRDS('analyses/results/confint_EEG_genderAgr_MiniNor_400_900_lateral_lmerTest.rds')

# Save random effects
lme4::ranef(EEG_genderAgr_MiniNor_400_900_lateral_lmerTest) %>%
  saveRDS('analyses/results/ranef_EEG_genderAgr_MiniNor_400_900_lateral_lmerTest.rds')
