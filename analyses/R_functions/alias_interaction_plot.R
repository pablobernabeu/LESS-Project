...
alias_interaction_plot = 
  ...
  function(model, dataset, x, fill, fill_alias, fill_nesting_factor = NULL, 
    ...
    setDT(as.data.table(model@frame))
    setDT(as.data.table(dataset))
    
    # Ensure unique mapping in dataset
    dataset <- unique(dataset, by = fill)
    
    # Perform fast lookup with match() to handle missing values safely
    matched_indices <- match(model@frame[[fill]], dataset[[fill]])
    
    # Only update non-missing matches and ensure values are characters
    valid_matches <- !is.na(matched_indices)
    model@frame[[fill_alias]][valid_matches] <- 
      as.character(dataset[[fill_alias]][matched_indices[valid_matches]])
    ...
    plot_model(model, type = 'pred', terms = c(x, fill), ci.lvl = .95) +
      ...
      scale_colour_discrete(aesthetics = c('colour', 'fill'), labels = fill_labels, 
      ...
      theme(plot.title = element_blank(), panel.grid.major = element_blank(), 
            ...
            axis.title.x = element_markdown(size = 12.5, margin = margin(t = 6)),
            axis.title.y = element_markdown(size = 12.5, margin = margin(r = 6)),
            axis.text = element_text(size = 11), 
            axis.line = element_line(colour = 'black'), 
            legend.title = element_markdown(
              size = 12.5, hjust = 0.5, vjust = 0.5, 
              margin = margin(b = 5), lineheight = 1.4
            ), 
            legend.title.align = 0.5, 
            # Adjust size of text depending on number of levels in fill_labels
            legend.text = element_markdown(
              size = ifelse(length(fill_labels) < 4, 11, 10.5), vjust = .5
            ), 
            ...
            legend.background = element_rect(colour = 'grey70', fill = 'transparent'),
