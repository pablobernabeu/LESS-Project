...
LHQ3_results_raw[1, 134] <- "L1_accent"
...
LHQ3_results_raw[1, 136] <- "L2_accent"
...
LHQ3_results_raw[1, 138] <- "L3_accent"
...
LHQ3_results_raw[1, 140] <- "L4_accent"
...
LHQ3_data_compact[, codeswitching] <- 
  lapply(LHQ3_data_compact[, codeswitching], function(x) {
    num_x <- as.numeric(as.character(x))
    return(num_x)
  })

# Step 3: Replace NA values with 0 in the converted columns
LHQ3_data_compact[, codeswitching][is.na(LHQ3_data_compact[, codeswitching])] <- 0

# Step 4: Calculate the row-wise average for the specified columns
LHQ3_data_compact$Codeswitching_average <- rowMeans(LHQ3_data_compact[, codeswitching], na.rm = TRUE)
...
names(LHQ3_processed)[c(1, 6, 7, 14)] <- c("Participant_ID", "L1_Proficiency", 
                                           "L2_Proficiency", 
...
LHQ3_processed_selected <- 
  LHQ3_processed[, c("Participant_ID", "L1_Proficiency", "L2_Proficiency", 
                     "Multilingual_Language_Diversity")]
